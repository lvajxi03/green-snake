...
Arena::Arena()
{
  ...
  HRSRC hFntRes = FindResource(hInstance, MAKEINTRESOURCE(IDF_EF64), L"BINARY");
  if (hFntRes)
  {
    ...
    if (hFntMem != nullptr)
    {
      ...
      _font_handle = AddFontMemResourceEx(FntData, len, nullptr, &nFonts);
    }
  }
...
Arena::~Arena()
{
  ...
  DeleteObject(_cup);
  RemoveFontMemResourceEx(_font_handle);
}
    ...
    LPARAM lParam)
{
  ...
  case WM_MOUSEHWHEEL:
  {
    game->keyup(GET_WHEEL_DELTA_WPARAM(wParam) < 0 ? KEY_LEFT : KEY_RIGHT);
  }
  break;
  case WM_MOUSEWHEEL:
  {
    game->keyup(GET_WHEEL_DELTA_WPARAM(wParam) > 0 ? KEY_UP : KEY_DOWN);
  }
  break;
  case WM_RBUTTONDOWN:
  {
